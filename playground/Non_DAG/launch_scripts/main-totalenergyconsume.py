...
from playground.Non_DAG.algorithm.DeepJS.reward_giver import TotalEnergyConsumeRewardGiver
...
brain = Brain(6)
reward_giver = TotalEnergyConsumeRewardGiver()
...
episode = Episode(machine_configs, jobs_configs, algorithm, "./random.json")
episode.run()
print(episode.env.now, time.time() - tic, average_completion(episode), average_slowdown(episode))

tic = time.time()
algorithm = FirstFitTaskalgorithm()
episode = Episode(machine_configs, jobs_configs, algorithm, "./ff.json")
episode.run()
print(episode.env.now, time.time() - tic, average_completion(episode), average_slowdown(episode))
    ...
    for itr in range(n_iter):
        ...
        for i in range(n_episode):
            algorithm = RLAlgorithm(agent, reward_giver, features_extract_func=features_extract_func,
                                    features_normalize_func=features_normalize_func)
            episode = Episode(machine_configs, jobs_configs, algorithm, "./drl.json")
