...
class Episode(object):
    ...
    def __init__(self, machine_configs, task_configs, task_algorithm, cooling_equipment=None, event_file=None):
        self.env = simpy.Environment()
        cluster = Cluster()
        cluster.add_machines(machine_configs)
        task_broker = Episode.broker_cls(self.env, task_configs)

        scheduler = Scheduler(self.env, task_algorithm, cooling_equipment)
