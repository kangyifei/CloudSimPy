...
for ttt in range(times):
    ...
    for i in range(n_iter):
        ...
        print(episode.env.now, time.time() - tic, average_completion_res, average_waitting_time_res,
              average_slowdown(episode))
        algorithm.reset()
    with open("./record_" + str(ttt) + ".csv", "w+", newline='') as f:
        header = ["total_energy_consume",
                  "total_called_num",
                  "total_reward",
                  "average_completion",
                  "average_waitting_time"]
        d = [{header[0]: total_energy_consume_list[i],
              header[1]: total_called_num_list[i],
              header[2]: total_reward_of_ep_list[i],
              header[3]: average_completion_list[i],
              header[4]: average_waitting_time_list[i]} for i in range(len(total_energy_consume_list))]
