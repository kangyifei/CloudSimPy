...
class Agent(object):
    ...
    def append_sample(self, state, action, reward, next_state, done):
        ...
        if len(self.memory) < MEMORY_SIZE:
            ...
            pointer = self.memory_pointer % MEMORY_SIZE
            self.memory[pointer] = (state, action, reward, next_state, done)
            self.memory_pointer += 1

    def sample(self, num):
        index = [random.randint(0, len(self.memory)-1) for _ in range(num)]
        batch = []
        for i in range(num):
            batch.append(self.memory[index[i]])
        return batch
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        if self.times % LEARN_FREQUENCY == 0:
            ...
            loss = self.loss_func(eval_q, target_q_actul)
