...
class DQNAlgorithm(Algorithm):
    ...
    def __call__(self, cluster, clock, cooling_equip=None):
        ...
        if not len(tasks) == 0:
            if self.task_pointer >= len(tasks):
                self.task_pointer = 0
            task_tobe_sched = tasks[self.task_pointer]
            observation = []
            for machine in machines:
                observation.append(machine.cpu)
                observation.append(machine.memory)
            observation.append(task_tobe_sched.task_config.cpu)
            observation.append(task_tobe_sched.task_config.memory)
            observation = np.array(observation)
            action = self.dqn.choose_action(observation)
            if not machines[action].accommodate(task_tobe_sched):
                reward = -0.1+cluster.monitor.time_penal
                cluster.monitor.time_penal=0
                ...
                reward = self.reward_giver(cluster, clock)+cluster.monitor.time_penal
                cluster.monitor.time_penal=0
                self.total_reward_of_ep += reward
                self.dqn.store_transition(self.last_observation, self.last_action, self.last_reward, observation)
                self.last_observation = observation
                ...
                self.steps += 1
                self.task_pointer = 0
                return machines[action], task_tobe_sched, None
        return None, None, None
