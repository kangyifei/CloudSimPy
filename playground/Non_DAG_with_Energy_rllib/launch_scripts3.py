...
import playground.Non_DAG_with_Energy.algorithm.DQN_noFCFS_makespan.DQN_algorithm as dqn_nofcfs_top
algorithm = dqn_nofcfs_top.DQNAlgorithm(machine_configs)
...
for i in range(n_iter):
    ...
    print(episode.env.now, time.time() - tic, average_completion_res, average_slowdown(episode))
    algorithm.reset()
...
plt.xlabel('l3:energy_consume/total_reward_of_ep')
plt.show()
