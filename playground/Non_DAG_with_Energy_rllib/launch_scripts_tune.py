...
os.environ['PYTHONPATH'] = os.environ['PYTHONPATH'] + ":" + os.path.abspath("../../..")
...
from playground.Non_DAG_with_Energy.algorithm.DQN_tune.DQN_algorithm import DQNAlgorithm
...
def train(config):
    ...
    np.random.seed(config.get("npseed"))
    tf.random.set_seed(config.get("tfseed"))
    algorithm = DQNAlgorithm(machine_configs, config)
    ...
    tune.report(tec=algorithm.total_energy_consume, tre=algorithm.total_reward_of_ep, ct=episode.env.now,
                ac=average_completion(episode), aw=average_waiting_time(episode), asl=average_slowdown(episode))
    print(episode.env.now, average_completion(episode), average_waiting_time(episode),
...
config = {"npseed": tune.sample_from(lambda _: np.random.randint(1, 100)),
          "tfseed": tune.sample_from(lambda _: np.random.randint(1, 100)),
          "lr": tune.uniform(0.2, 0.001),
          "l1": tune.grid_search([15, 20, 25, 30]),
          "l2": tune.grid_search([20, 25, 30, 35]),
          "l3": tune.grid_search([25, 30, 35, 40]),
          "l4": tune.grid_search([20, 15, 30, 35]), }
tune.run(train, config=config, num_samples=400, )
