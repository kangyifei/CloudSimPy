...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        self.q_target = tf.compat.v1.placeholder(tf.float32, [None, self.n_actions], name='Q_target')  # for calculating loss 用来接收q_target的值
        with tf.compat.v1.variable_scope('eval_net'):
            ...
            with tf.compat.v1.variable_scope('l5'):
                w5 = tf.compat.v1.get_variable('w5', [20, self.n_actions], initializer=w_initializer, collections=c_names)
                b5 = tf.compat.v1.get_variable('b5', [1, self.n_actions], initializer=b_initializer, collections=c_names)
